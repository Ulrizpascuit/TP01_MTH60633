...
```
La figure suivante (figure 3) présente les résultats finaux. On y voit les log-rendements des deux indices ainsi que leur VaR respective.
...
```
Visuellement, on peut dire que les VaR calculées ont l'air acceptable et cohérentes avec le movement des log-rendements. En effet, on observe que la VaR varie dans le temps en fonction de la volatilité conditionnelle estimée par le modèle GARCH(1,1). Lorsque les log-rendements présentent une plus grande variabilité, la variance conditionnelle augmente, ce qui se traduit par une VaR plus négative et donc par une perte potentielle plus importante. À l’inverse, durant les périodes de volatilité plus faible, la VaR se rapproche de zéro.
...
```
On voit que l'occurence de dépassement de la VaR est proche de 5%, mais tout de même légèrement au dessus de 5%. Il est possible de dire qu'un échantillon de 1000 données n'est peut être pas assez grand et cela pourrait être la cause de ces deux valeurs supérieurs à la valeur théorique. Cependant, d'un point de vu financier, ce résultat est loin d'être étonnant. En effet, on parle souvent que le modèle normal appliqué à des rendements de séries n'est pas bon pour bien capter le comportement de valeurs extrêmes. Ici on s'intéresse aux gros rendements négatifs. Le modèle normal va nécessairement sous-estimer la probabilité d'occurence de ce genre d'évènement. C'est pourquoi qu'en pratique, on observe plus de log-rendements qui dépasse la VaR à 5% que prévu. On pourrait même discuter de l'asymmétrie (skewness) des données qui pourraient aussi avoir un impact, car le modèle normal est symmétrique. Cela étant dit, on voit que le GARCH(1,1) donne une bonne allure générale de la VaR estimée à un pas futur, mais c'est loin d'être le modèle le plus adéquat à utiliser.
