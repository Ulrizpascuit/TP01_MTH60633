...
Ce modèle impose des contraintes d,a,b ≥ 0 donc on impose une borne 1e−8 (plutôt que 0) pour éviter les cas limites qui peuvent rendre l’optimisation instable. On impose aussi la stationnarité, via a+b < 1−ε, nécessaire pour que la variance reste finie et que la récurrence soit stable. Ces contraintes sont imposées dans A et b, puis passées au solveur lors de l’estimation.
Une fois les paramètres estimés, on recalcule la suite des variances conditionnelles dans f_ht ; l’initialisation se fait avec la variance inconditionnelle d/(1−a−b). Enfin, la VaR correspond au quantile inférieur de la distribution conditionnelle, ici distribuée normalement grace au maximum de vraisemblance. On extrait donc le quantile visé, ici les 5% les plus bas (VaR 95%).
